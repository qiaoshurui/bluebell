...
func Init() (err error) {
	...
	//if model == "dev" { //开发阶段输出到终端上。
	//	consoleEnbcoder := zapcore.NewConsoleEncoder(zap.NewDevelopmentEncoderConfig())
	//	core = zapcore.NewTee(
	//		zapcore.NewCore(encoder, writeSyncer, l),                                      //将错误信息输入到指定的文件中
	//		zapcore.NewCore(consoleEnbcoder, zapcore.Lock(os.Stdout), zapcore.DebugLevel), //通过下面的终端输出运行时出现的错误。
	//	)
	//} else {
